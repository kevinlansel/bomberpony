...
MyGame::MyGame(bool sound, const std::list<std::string> &toload) : Scene(Vector3f(0, 0, 0), Vector3f(0, 0, 0)), _error(false)
{
...
void		MyGame::initialize(const std::string &map_type)
{
  Map				map;
...
MenuType	MyGame::update(gdl::GameClock &clock, gdl::Input &input)
{
...
void		MyGame::newPlayer(unsigned int size_list, unsigned int x, unsigned int y)
{
